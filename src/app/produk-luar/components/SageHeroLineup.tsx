...
export default function SageHeroLineup() {
  ...
  return (
    <section className="relative overflow-hidden bg-surface pt-12 pb-20 lg:pb-24">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 mb-12">
          {featured.map(({ product, badge, badgeTone }) => {
            // Pull the portrait card asset via the shared config so this
            // surface stays in sync with ProductCard. Falls back to the
            // legacy mainImage if a product isn't in the config map.
            const cardImg = getCardImage(product.slug);
            return (
              <Link
                key={product.slug}
              ...
              >
                ...
                <div className="relative aspect-[4/5] flex items-center justify-center p-8">
                  {cardImg ? (
                    // Above-fold featured pair: eager + high priority.
                    // object-contain keeps the letterbox aesthetic that
                    // differs from the lower grid (which uses object-cover).
                    <picture>
                      <source
                        media="(max-width: 768px)"
                        srcSet={cardImg.mobile}
                      />
                      ...
                      <img
                        ...
                        fetchPriority="high"
                        decoding="async"
                        className="absolute inset-0 w-full h-full object-contain p-10 transition-transform duration-500 group-hover:scale-[1.04]"
                      />
                    </picture>
                  ) : (
                    <Image
